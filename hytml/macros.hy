...
(eval-and-compile
  ...
  ; recursively calling the same parser on content generation
  ...
  (defmacro defparse [name do-code get-content]
    `(defn ~name [code] (if (coll? code)
      (do
        ...
        (if ; comment tag <!-- -->
            ...
            ; both ~@(list-comp) and ~@(for-list) functions
            (= tag "unquote_splice")
  ...
  (defmacro defparsehtml [parser specs get-content e]
    `(defparse ~parser
      (if (tag? tag ~specs)
          (do
            (defsets ~get-content)
            ; empty content will give a short tag
            (+ (tag-start tag (get-attributes content-code) (empty? content) (omit? tag ~specs))
               (if (and (not (forbidden? tag ~specs)) (not (empty? content)))
                   (+ (tag-content content) (tag-end tag))
                   "")))
          (do (raise (HyTMLError (% "Tag '%s' not meeting %s specs" (, tag ~e)))) "")) ~get-content))
  
  (defparsehtml parse-html4 specs4 get-content4 "html4")
  (defparsehtml parse-html5 specs5 get-content5 "html5")

  (defmacro defparsexhtml [parser specs get-content e]
    `(defparse ~parser
      (if (tag? tag ~specs)
          (do
            (defsets ~get-content)
  ...
  (defparsexhtml parse-xhtml4 specs4 get-content4x "xhtml4")
  (defparsexhtml parse-xhtml5 specs5 get-content5x "xhtml5")
  ...
  (defn tag-start [name attr &optional [short False] [omit False]]
    (+ "<" (str name) (concat-attributes attr) (if (and short (not omit)) "/>" ">")))

  (defn tag-end [name] (+ "</" (str name) ">")))
