...
;         (h1 ~(.capitalize "page header"))
;         (ul "List"
;           (for-each [i [1 2 3]] `(li ~i)))))))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(import (hytml.specs (specs4 specs5 specs5!)))
...
(eval-and-compile
  ...
  ; this maneuver is needed so that parser entry point keeps
  ; recursively calling same parser on content generation
  (defcontent get-content parse-xml)
  (defcontent get-content4 parse-html4)
  (defcontent get-content5 parse-html5)
  (defcontent get-content4x parse-xhtml4)
  (defcontent get-content5x parse-xhtml5)
  ...
  ;(defget get-content-attributes5 parse-html5)
  ;(defget get-content-attributes4x parse-xhtml4)
  ...
  (defmacro defparse [name do-code get-content-version]
    `(defn ~name [code]
      (if (coll? code)
      (do
        ...
        (if ; comment tag <!-- -->
            ...
            (= tag "!__")
            (+ "<!--" (tag-content (~get-content-version (list (drop 1 code)))) "-->")
            ...
            (= tag "!DOCTYPE")
            (+ "<!DOCTYPE " (tag-content (~get-content-version (list (drop 1 code)))) ">")
            ...
            (= tag "unquote_splice")
            (.join "" (map ~name (eval (second code))))
  ...
  (defn tag? [tag specs] (in (keyword tag) specs))
  (defn tag-specs [tag specs] (get specs (keyword tag)))
  ...
  (defparse parse-html4
    ...
    (do
      (if (tag? tag specs4)
          (do
            (setv content-code (list (drop 1 code)))
            (setv content (get-content4 content-code))
            ; empty content will give a short tag
            (+ (tag-start tag (get-attributes content-code) (empty? content) (omit? tag specs4))
               (if (and (not (forbidden? tag specs4)) (not (empty? content)))
                   (+ (tag-content content) (tag-end tag))
                   "")))
          (raise (Exception (% "Tag '%s' not meeting html4 specs" tag))))) get-content4)

  (defparse parse-html5
    ; normal tag, attribute, and content processing
    (do
      (if (tag? tag specs5)
          (do
            (setv content-code (list (drop 1 code)))
            (setv content (get-content5 content-code))
            ; empty content will give a short tag
            (+ (tag-start tag (get-attributes content-code) (empty? content) (omit? tag specs5))
               (if (and (not (forbidden? tag specs5)) (not (empty? content)))
                   (+ (tag-content content) (tag-end tag))
                   "")))
          (raise (Exception (% "Tag '%s' not meeting html5 specs" tag))))) get-content5)

  (defparse parse-xhtml4
    ; normal tag, attribute, and content processing
    (do
      (if (tag? tag specs4)
          (do
            (setv content-code (list (drop 1 code)))
            (setv content (get-content4x content-code))
            ; empty content will give a short tag
            (+ (tag-start tag (get-attributes content-code) (empty? content) False)
               (if (and (not (forbidden? tag specs4)) (not (empty? content)))
                   (+ (tag-content content) (tag-end tag))
                   (not (empty? content))
                   (tag-end tag)
                   "")))
          (raise (Exception (% "Tag '%s' not meeting xhtml5 specs" tag))))) get-content4x)

  (defparse parse-xhtml5
    ; normal tag, attribute, and content processing
    (do
      (if (tag? tag specs5)
          (do
            (setv content-code (list (drop 1 code)))
            (setv content (get-content5x content-code))
            ; empty content will give a short tag
            (+ (tag-start tag (get-attributes content-code) (empty? content) False)
               (if (and (not (forbidden? tag specs5)) (not (empty? content)))
                   (+ (tag-content content) (tag-end tag))
                   ...
                   (tag-end tag)
                   "")))
          (raise (Exception (% "Tag '%s' not meeting xhtml5 specs" tag))))) get-content5x)
  ...
  ; if value and attribute name is same. for example:
